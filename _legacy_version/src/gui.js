...
import { exportFingerprintTexture } from './pavilion/fingerprint.js';

export function setupGUI(params, callbacks) {
  const gui = new GUI({ width: 320, title: 'EXPO 2030 — Pavilion UA' });

  const { onParamChange, onScreenshot, onExportGLTF, onExportOBJ } = callbacks;
  ...
  const fpExportActions = {
    exportTexture() { exportFingerprintTexture(params); },
  };
  fp.add(fpExportActions, 'exportTexture').name('Export Texture (PNG)');
